...
// @ts-ignore
import { TreasureHuntContract } from '../src/artifacts/TreasureHunt.ts';
...
async function deployContract(wallet: Wallet, deployer: AztecAddress) {
  ...
  const deployMethod = new DeployMethod(
    ...
    TreasureHuntContract.artifact,
    (instance, wallet) =>
      TreasureHuntContract.at(instance.address, wallet),
    [deployer.toField()],
    getDefaultInitializer(TreasureHuntContract.artifact)?.name
  );
  ...
  await deployMethod
    ...
    .wait({ timeout: 120 });
  await wallet.registerContract(contract, TreasureHuntContract.artifact);
...
async function writeEnvFile(deploymentInfo: { contractAddress: string; deployerAddress: string; deploymentSalt: string }) {
  ...
  console.log(`
      \n\n\n
      TreasureHunt contract deployed successfully. Config saved to ${envFilePath}
