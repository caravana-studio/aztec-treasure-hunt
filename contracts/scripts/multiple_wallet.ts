...
const setupWallet1 = async () => {
    return await EmbeddedWallet.create(node, {
        ephemeral: false,
        pxeConfig: { proverEnabled: getEnv() !== 'local-network' },
    });
};
...
async function main() {
    ...
    let salt = Fr.random();
    let schnorrAccount = await AccountManager.create(wallet1, secretKey, new SchnorrAccountContract(signingKey), salt);
    ...
    let salt2 = Fr.random();
    let schnorrAccount2 = await AccountManager.create(wallet2, secretKey2, new SchnorrAccountContract(signingKey2), salt2);
